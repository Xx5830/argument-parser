...
**nargparse** - это современная C++ библиотека для разбора аргументов командной строки. Поддерживает флаги, опциональные и обязательные аргументы, позиционные параметры, повторяющиеся значения и пользовательские валидаторы. Вся мощь шаблонов и type-safe API без макросов и скрытого состояния.
